		...
		return dispatch(
			morseCall({
				data: {
					...
					frequency: s.user.settings.key_frequency,
...
const chatSlice = createSlice({
	...
	reducers: {
		...
		setChannelByName(state, action) {
			let filtered = state.channels.filter(
		...
		setAllowSound(state, action) {
			if (state.onlineUsers[action.payload.user]) {
				state.onlineUsers[action.payload.user].allowSound =
					action.payload.allowSound;
			}
		},
	},
...
export function selectChannelName(state) {
	...
	let filtered = state.chat.channels.filter(
		(current) => current.ch == selectedChannel,
	);
	if (filtered.length > 0) {
